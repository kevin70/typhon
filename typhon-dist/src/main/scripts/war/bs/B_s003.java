 ...
 */
public class B_s003 extends BSaScript {
    ...
    protected Object doExecute(BSaWapper bsaWapper) {
        ...
        int shieldHp = (int) (aobj.getMaxHp() * 0.15);
        aobj.setShieldHp(shieldHp);

        goal.decrementHp(hp);
        mar.addTarget(ae);

        if (aobj.getLab() != FightObject.YHU_FIGHT_OBJECT_LAB) {
...
//            br.setBufid(BufferSkill.SHIELD_BUF);
//            AttackEntry reae = new AttackEntry();
//            reae.setLab(aobj.getLab());
//            reae.setVal(shieldHp);
//            br.setSource(reae);
            mar.addTarget(re);
        }
        return mar;
