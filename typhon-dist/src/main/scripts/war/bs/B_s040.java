...
import static org.skfiy.typhon.spi.war.BufferSkill.POISON_BUF;
 ...
 */
public class B_s040 extends BSaScript {
    ...
    protected Object doExecute(BSaWapper bsaWapper) {
        ...
        for (FightObject goal : bsaWapper.getDefenderEntity().findFightGoals(5)) {
    ...
    private class MyBufferSkill implements BufferSkill {
        ...
        public Object onAfter() {
            if (warInfo.getRound() - beginRound >= 5) {
                onFinish();
            }

            // DEBUF造成的伤害
            int d = (int) (damage * 0.2);
            fobj.decrementHp(d);

            // 返回
            BufferResult br = new BufferResult();
            br.setBufid(POISON_BUF);
            br.setRound(5 - (warInfo.getRound() - beginRound));
