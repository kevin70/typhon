 ...
 */
public class B_s064 extends BSaScript {
    ...
    protected Object doExecute(BSaWapper bsaWapper) {
        ...
        for (FightObject obj : bsaWapper.getAttackerEntity().getFightObjects()) {
            if (!obj.isDead()) {
                myFuries += obj.getFury();
            }
        }

        for (FightObject obj : bsaWapper.getDefenderEntity().getFightObjects()) {
            if (!obj.isDead()) {
                otherFuries += obj.getFury();
            }
        }
