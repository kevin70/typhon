 ...
 */
public class B_s017 extends BSaScript {
    ...
    protected Object doExecute(BSaWapper bsaWapper) {
        ...
        // 找出血量最低的人
        WarInfo.Entity entity = bsaWapper.getAttackerEntity();
        ...
        for (FightObject goal : bsaWapper.getDefenderEntity().findFightGoals(bsaSkill.getNumOfTargets())) {
            ...
            for (FightObject fo : entity.getFightObjects()) {
                if (fo.isDead()) {
                    continue;
                }
                ...
                if (fo.getHp() * 1D / fo.getMaxHp() < lowest.getHp() * 1D / fo.getMaxHp()) {
                    lowest = fo;
                }
            }
