 ...
 */
public class L_s068 implements Script {

    @Override
    public Object invoke(Session session, Object obj) {
        List<FightObject> objs = (List<FightObject>) obj;
        for (FightObject fo : objs) {
            if (fo.getArea() == Area.Shu || fo.getArea() == Area.Wu) {
                fo.setMaxCritRate(fo.getMaxCritRate() + 0.05);
                fo.setMaxMatk(fo.getMaxMatk() + (int) (fo.getMaxMatk() * 0.05));
            }
        }
        return null;
    }
